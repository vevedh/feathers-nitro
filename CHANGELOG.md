...
- Stabilized the StackBlitz WebContainer bootstrap around pinned `pnpm@10.34.5`: pnpm is installed under `$HOME/.local`, self-managed package-manager relaunches are disabled, optional pnpm hook probing is disabled, workspace `.npmrc` placeholders stay empty, and the repository lockfile remains frozen.
